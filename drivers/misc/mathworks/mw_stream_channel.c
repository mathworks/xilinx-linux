...
static long mwadma_rx_ctl(struct mwadma_dev *mwdev, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case MWADMA_RX_GET_NEXT_INDEX:
            ...
            if(copy_to_user((unsigned long *) arg, &next_index, sizeof(next_index))) {
                return -EACCES;
            }
            break;
        case MWADMA_RX_GET_ERROR:
            if(copy_from_user(&done_index, (unsigned long *)arg, sizeof(done_index))) {
                return -EACCES;
            }
            spin_lock_irqsave(&mwchan->slock, flags);
            mwchan->transfer_queued--;
            ...
            if(copy_to_user((unsigned long *) arg, &error, sizeof(error))) {
                return -EACCES;
            }
            break;
